...
namespace samples {
                        ...
                        int materialID)
  {
    if (sphereRecDepth <= 0)
      // use a real sphere
      spheres.push_back({center,radius,materialID});
    else
  ...
  void World::finalize()
  {
    std::cout << "#prime-cuda: done w/ creating input scene, found "
              << prettyNumber(triangles.size()) << " triangles and " 
              << prettyNumber(spheres.size()) << " spheres" << std::endl;
    context = opContextCreate(OP_CONTEXT_GPU,0);
    assert(context);

    std::vector<OPGeom> geoms;
    if (!spheres.empty()) {
      OPGeom geom = opSpheresFromArrays(context,
                                        /* userID=1 : spheres */1,
                                        spheres.size(),
                                        &spheres[0].center.x,
                                        &spheres[0].center.y,
    ...
    if (!triangles.empty()) {
      OPGeom geom = opMeshFromTriangles(context,
                                        /* userID==0: triangles*/0,
                                        triangles.size(),
                                        &triangles[0].A.x,
                                        &triangles[0].A.y,
                                        &triangles[0].A.z,
    ...
    model = opModelFromGeoms(context,geoms.data(),geoms.size());
    assert(model);
  }
