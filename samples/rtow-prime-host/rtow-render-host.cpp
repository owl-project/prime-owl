...
#define RANDVEC3F vec3f(rnd(),rnd(),rnd())
  ...
  void World::addSphere(const vec3f center, const float radius, Material *material, int recDepth)
  {
          ...
          for (int depth=0;depth<maxPathLength;depth++) {
            ...
            array2D::for_each(vec2i(TILE_SIZE),[&](const vec2i &l_pixelID){
                ...
                if (isecs[lID].primID < 0) {
                  if (depth == 0) {
                    const vec2i pixelID = tileID*TILE_SIZE+l_pixelID;
                    weight[lID] = missColor(normalize(rays[lID].direction));//pixelID,fb->size);
                  }
                  active[lID] = false;
                  rays[lID].tmin = 0.f;
                  rays[lID].tmax = 0.f;
...
#endif
                if (tri.material->scatter(scatter,random)) {
                  numActive++;
                  org = scatter.out_org;
                  dir = scatter.out_dir;
                  rays[lID].tmin = 1e-3f;
